...
- Injected runtime secrets (`OPENAI_API_KEY`, `OPENAI_PROJECT_ID`, `FLY_API_TOKEN`) are provided as process env vars. `.env.local` intentionally omits `OPENAI_API_KEY`/`OPENAI_PROJECT_ID` so it cannot mask them. Note: the portal `tmux` server must be started after secrets are injected (restart it with `tmux kill-server` if a session lacks them) or long-running processes launched from it will not see the secrets.
