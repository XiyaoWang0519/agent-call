...
- Listen to the callee's greeting or menu before speaking, with a 1.5-second silence
  fallback. Enable turn-taking without waiting for asynchronous AMD; late voicemail
  detection suspends and cancels native replies before the voicemail response.
- Preserve the far-field input filter across session updates. Default to server VAD
  with 300 ms silence and threshold 0.5; semantic VAD remains configurable.
