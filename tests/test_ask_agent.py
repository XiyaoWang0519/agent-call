...
async def _answer(service, call_id: str, question_id: str, answer: str, **kwargs):
    """Submit an answer through the typed call-entry adapter (R06)."""

    return await service.answer_call_question(
        AnswerCommand(call_id=call_id, question_id=question_id, answer=answer, **kwargs)
    )
...
async def test_ask_persists_without_immediate_tool_output(ask_service, packet):
    ...
    assert not any(result[1] == "tc_1" for result in ask_service.live.tool_results)
...
async def test_mcp_rejects_unsupported_not_found_without_claiming_question(ask_service, packet):
    ...
    assert not any(
        ...
        for _, tool_call_id, _ in ask_service.live.tool_results
    )
...
async def test_answer_delivers_correlated_output_and_continuation(ask_service, packet):
    ...
    result = await _answer(ask_service, call_id, question_id, "CVS on Market Street")
    await wait_background()

    assert result["status"] == "accepted"
    assert result["question_id"] == question_id
    assert "wait_for_call_event" in result["next_action"]
    assert ask_service.live.tool_results[-1] == (
        call_id,
        "tc_1",
        {"status": "answered", "answer": "CVS on Market Street"},
    )
    text = ask_service.live.tool_result_continuation_texts[-1]
...
async def test_timeout_exactly_once_then_answer_is_expired(ask_service, packet):
    ...
    timeout_results = [r for r in ask_service.live.tool_results if r[1] == "tc_timeout"]
    ...
    late = await _answer(ask_service, call_id, question_id, "too late")
    await wait_background()
    ...
    assert len([r for r in ask_service.live.tool_results if r[1] == "tc_timeout"]) == 1
...
async def test_answer_timeout_race_exactly_one_output(ask_service, packet):
    ...
    answer_task = asyncio.create_task(
        _answer(ask_service, call_id, question_id, "race winner answer")
    )
    await asyncio.sleep(0.12)
    await answer_task
    await wait_background()

    correlated = [r for r in ask_service.live.tool_results if r[1] == "tc_race"]
...
async def test_duplicate_answer_idempotent_and_unknown_question_errors(ask_service, packet):
    ...
    first = await _answer(ask_service, call_id, question_id, "first")
    second = await _answer(ask_service, call_id, question_id, "second")
    await wait_background()
    ...
    assert len([r for r in ask_service.live.tool_results if r[1] == "tc_1"]) == 1

    with pytest.raises(LookupError, match="unknown question"):
        await _answer(ask_service, call_id, "missing_qid", "nope")
    with pytest.raises(LookupError, match="unknown question"):
        await _answer(ask_service, "other_call", question_id, "nope")


@pytest.mark.asyncio
async def test_mcp_unknown_question_is_a_structured_code(ask_service, packet):
    """R06: the last unstructured ToolError now carries a stable code."""

    call_id = await seed_call(ask_service.db, packet, state=CallState.ACTIVE)
    mcp = FastMCP("test-ask-agent-unknown-question")
    register_tools(mcp, lambda: ask_service)
    ...
    assert payload["code"] == "unknown_question"
    assert "unknown question" in payload["message"]
...
async def test_second_ask_while_pending_returns_error_and_keeps_call_active(ask_service, packet):
    ...
    second = [r for r in ask_service.live.tool_results if r[1] == "tc_b"]
    assert len(second) == 1
    assert second[0][2] == {"status": "error", "error": "question_pending"}
    assert not any(r[1] == "tc_a" for r in ask_service.live.tool_results)
    call = await ask_service.db.get_call(call_id)
...
async def test_question_limit_returns_error(ask_service, packet):
    ...
    rows = await ask_service.db.get_questions_after(call_id, 0)
    await _answer(ask_service, call_id, rows[0]["question_id"], "done")
    await wait_background()
    ...
    await wait_background()
    limited = [r for r in ask_service.live.tool_results if r[1] == "tc_limit_2"]
...
async def test_termination_cancels_pending_and_late_answer_is_call_ended(ask_service, packet):
    ...
    rows = await ask_service.db.get_questions_after(call_id, 0)
    question_id = rows[0]["question_id"]
    before = len(ask_service.live.tool_results)
    ...
    assert call_id not in ask_service._pending_questions
    assert len(ask_service.live.tool_results) == before

    late = await _answer(ask_service, call_id, question_id, "after end")
...
async def test_ask_agent_while_voice_end_pending_returns_call_ending(ask_service, packet):
    ...
    result = [r for r in ask_service.live.tool_results if r[1] == "tc_ending"]
...
async def test_ask_agent_disabled_returns_error_and_tool_absent_from_payload(
    ...
    await wait_background()
    result = [r for r in service.live.tool_results if r[1] == "tc_disabled"]
    ...
    names = [
        tool.name for tool in bridge.build_accept_payload(packet).session.delegation.responses.tools
    ]
...
async def test_answer_rejected_while_call_terminating(ask_service, packet):
    ...
    result = await _answer(ask_service, call_id, question_id, "too late")
    await wait_background()

    assert result["status"] == "call_ended"
    row = await ask_service.db.get_question(question_id)
    assert row["status"] == "pending"
    assert not any(r[1] == "tc_term_race" for r in ask_service.live.tool_results)
...
@pytest.mark.asyncio
async def test_answer_delivery_preserves_live_backend_response(ask_service, packet):
    call_id = await seed_call(ask_service.db, packet, state=CallState.ACTIVE)
    await _ask(ask_service, call_id, tool_call_id="tc_cancel")
    rows = await ask_service.db.get_questions_after(call_id, 0)
    ask_service._active_response_ids[call_id] = "resp_active"

    await _answer(ask_service, call_id, rows[0]["question_id"], "the answer")
    await wait_background()

    assert ("cancel_response", call_id) not in ask_service.live.events
    assert ask_service._active_response_ids[call_id] == "resp_active"
    delivered = [r for r in ask_service.live.tool_results if r[1] == "tc_cancel"]
    assert len(delivered) == 1
    assert delivered[0][2]["status"] == "answered"


@pytest.mark.asyncio
async def test_end_call_refuses_to_abandon_pending_question(ask_service, packet):
    ask_service.settings.ask_agent_answer_timeout_seconds = 30.0
    call_id = await seed_call(ask_service.db, packet, state=CallState.ACTIVE)
    ...
    await ask_service.handle_live_event(
        call_id,
        _tool_event(
            "tc_end",
            "finish_call_after_goodbye",
            '{"reason": "objective_completed", "farewell": "Goodbye"}',
        ),
    )
    await wait_background()
    ...
    assert row["status"] == "pending"
    assert call_id in ask_service._pending_questions
    assert not any(r[1] == "tc_pending" for r in ask_service.live.tool_results)

    late = await _answer(ask_service, call_id, question_id, "too late")
    await wait_background()
    assert late["status"] == "accepted"
    assert any(r[1] == "tc_pending" for r in ask_service.live.tool_results)
...
@pytest.mark.asyncio
async def test_duplicate_tool_call_id_keeps_original_result(ask_service, packet):
    call_id = await seed_call(ask_service.db, packet, state=CallState.ACTIVE)
    await _ask(ask_service, call_id, tool_call_id="tc_dup")
    rows = await ask_service.db.get_questions_after(call_id, 0)
    await _answer(ask_service, call_id, rows[0]["question_id"], "first answer")
    await wait_background()
    ...
    results = [r for r in ask_service.live.tool_results if r[1] == "tc_dup"]
    assert len(results) == 1
    assert results[-1][2] == {"status": "answered", "answer": "first answer"}
    assert await ask_service.db.count_call_questions(call_id) == 1
...
async def test_redelivered_pending_ask_is_idempotent(ask_service, packet):
    ...
    rows = await ask_service.db.get_questions_after(call_id, 0)
    question_id = rows[0]["question_id"]
    before_results = list(ask_service.live.tool_results)
    ...
    assert ask_service.live.tool_results == before_results
    assert await ask_service.db.count_call_questions(call_id) == 1
    assert call_id in ask_service._pending_questions
    assert ask_service._pending_questions[call_id].question_id == question_id

    accepted = await _answer(ask_service, call_id, question_id, "Walgreens")
    await wait_background()
    assert accepted["status"] == "accepted"
    delivered = [r for r in ask_service.live.tool_results if r[1] == "tc_redeliver"]
    assert len(delivered) == 1
...
async def test_redelivered_pending_ask_at_question_limit_is_idempotent(ask_service, packet):
    ...
    await wait_background()
    assert accepted["status"] == "accepted"
    delivered = [r for r in ask_service.live.tool_results if r[1] == "tc_quota_redeliver"]
    assert len(delivered) == 1
...
async def test_expiry_does_not_claim_once_call_is_terminating(ask_service, packet):
    ...
    assert not any(r[1] == "tc_expire_term" for r in ask_service.live.tool_results)
    row = await ask_service.db.get_question(question_id)
    assert row["status"] == "pending"
    assert call_id not in ask_service._pending_questions
...
async def test_failed_answer_delivery_is_retryable(ask_service, packet):
    ...
    ask_service.live.tool_result_failures_remaining = 1
    first = await _answer(ask_service, call_id, question_id, "CVS on Market")
    await wait_background()

    assert first["status"] == "accepted"
    assert not any(r[1] == "tc_retry" for r in ask_service.live.tool_results)
    row = await ask_service.db.get_question(question_id)
    assert row["status"] == "answered"

    retry = await _answer(ask_service, call_id, question_id, "ignored retry text")
    await wait_background()

    assert retry["status"] == "accepted"
    assert retry["question_id"] == question_id
    assert "wait_for_call_event" in retry["next_action"]
    delivered = [r for r in ask_service.live.tool_results if r[1] == "tc_retry"]
    assert len(delivered) == 1
    ...
    third = await _answer(ask_service, call_id, question_id, "again")
    await wait_background()
    assert third["status"] == "already_answered"
    assert "wait_for_call_event" in third["next_action"]
    assert len([r for r in ask_service.live.tool_results if r[1] == "tc_retry"]) == 1
...
async def test_ask_agent_lifecycle_emits_trace_logs(ask_service, packet, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="app.call_state"):
        ...
        answered = await _answer(ask_service, call_id, question_id, "CVS on Market Street")
        await wait_background()
