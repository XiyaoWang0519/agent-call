...
from app.mcp_oauth.constants import MCP_OAUTH_CONSENT_PATH, MCP_OAUTH_SCOPE, OAUTH_MCP_PATH
from app.mcp_oauth.provider import oauth_mcp_resource
from app.mcp_oauth.registration import is_valid_pkce_s256_challenge
...
def test_oauth_disabled_does_not_require_oauth_settings(settings):
    ...
    with TestClient(app) as client:
        ...
        assert client.post("/connect/mcp/").status_code == 404
        assert client.get("/.well-known/oauth-authorization-server").status_code == 404
        assert client.get("/.well-known/oauth-protected-resource/connect/mcp/").status_code == 404
...
def test_enabled_oauth_missing_settings_fail_startup(settings):
    ...
    with pytest.raises(ValidationError, match="MCP_OAUTH_ENABLED requires"):
        Settings(**values)


def test_invalid_owner_hash_fails_startup(oauth_settings):
    values = oauth_settings.model_dump()
    values["mcp_oauth_owner_secret_hash"] = SecretStr("not-a-real-hash")
...
def test_unsafe_ttl_values_fail_startup(oauth_settings):
    values = oauth_settings.model_dump()
    values["mcp_oauth_access_token_ttl_seconds"] = 86400
    with pytest.raises(ValidationError, match="ACCESS_TOKEN_TTL"):
        Settings(**values)
    values = oauth_settings.model_dump()
    values["mcp_oauth_refresh_token_ttl_days"] = 365
    with pytest.raises(ValidationError, match="REFRESH_TOKEN_TTL"):
        Settings(**values)
    values = oauth_settings.model_dump()
    values["mcp_oauth_auth_code_ttl_seconds"] = 3600
...
def test_oauth_secrets_are_redacted_from_repr_and_validation_errors(oauth_settings):
    ...
    values = oauth_settings.model_dump()
    values["mcp_oauth_signing_key"] = SecretStr("short")
    with pytest.raises(ValidationError) as exc:
        Settings(**values)
    error_text = str(exc.value)
    assert "MCP_OAUTH_SIGNING_KEY" in error_text
    assert secret not in error_text
    assert MCP_OAUTH_OWNER_SECRET not in error_text
    assert MCP_OAUTH_OWNER_SECRET_HASH not in error_text
    values = oauth_settings.model_dump()
    values["mcp_oauth_signing_key"] = SecretStr(secret)
    updated = Settings(**values)
    assert secret not in repr(updated)
    assert MCP_OAUTH_OWNER_SECRET not in repr(updated)
    assert MCP_OAUTH_OWNER_SECRET_HASH not in repr(updated)


def test_unauthorized_oauth_mcp_returns_401_with_resource_metadata(oauth_settings):
    app = create_app(oauth_settings)
    resource = oauth_mcp_resource(oauth_settings.public_base_url or "")
    with TestClient(app) as client:
        response = client.post(
            OAUTH_MCP_PATH, json={"jsonrpc": "2.0", "id": 1, "method": "tools/list"}
        )
        assert response.status_code == 401
        www = response.headers["www-authenticate"]
        assert "Bearer" in www
        assert "resource_metadata=" in www
        assert "/.well-known/oauth-protected-resource/connect/mcp/" in www
        metadata = client.get("/.well-known/oauth-protected-resource/connect/mcp/")
...
def _assert_pkce_rejected(client: TestClient, response) -> None:
    ...
    body = response.text
    assert MCP_OAUTH_CONSENT_PATH not in location
    assert MCP_OAUTH_CONSENT_PATH not in body
    ...
    if response.status_code == 302:
        ...
        if location.startswith("/"):
            ...
            assert MCP_OAUTH_CONSENT_PATH not in followed.headers.get("location", "")
        return
...
def test_wrong_client_redirect_and_resource_fail(oauth_settings):
    ...
    with TestClient(app) as client:
        ...
        if (parsed_location.scheme, parsed_location.netloc) == ("https", "client.example"):
            assert "error=" in location
        else:
...
def test_registrations_and_refresh_survive_restart(oauth_settings):
    ...
    with TestClient(restarted) as client:
        looked_up = restarted.state.mcp_oauth
        stored = client.post(
            "/token",
            data={
                "grant_type": "refresh_token",
        ...
        assert looked_up is restarted.state.mcp_oauth
...
def test_oauth_logs_do_not_contain_secrets(oauth_settings, caplog):
    ...
    assert MCP_OAUTH_OWNER_SECRET not in combined
...
def test_public_base_url_change_keeps_encrypted_oauth_storage(oauth_settings):
    ...
    with TestClient(restarted) as client:
        ...
        authorize = start_authorization(
            ...
            redirect_uri="https://client.example/callback",
            challenge=challenge,
            resource=oauth_mcp_resource(rotated.public_base_url or ""),
        )
        assert authorize.status_code == 302
        assert MCP_OAUTH_CONSENT_PATH in authorize.headers["location"]
...
def test_wrong_storage_key_fails_closed(oauth_settings):
    ...
    poisoned_values["mcp_oauth_storage_encryption_key"] = SecretStr("z" * 64)
...
def test_stored_oauth_rows_are_hashed_or_encrypted(oauth_settings):
    ...
    try:
        for table in (
            ...
            assert MCP_OAUTH_OWNER_SECRET not in serialized
...
def test_hash_script_imports_app_without_repo_root_on_path():
    ...
    script = repo / "scripts" / "hash_mcp_oauth_owner_secret.py"
    saved_modules = {
        name: module
...
def test_registration_rejects_oversized_body_and_metadata_before_persist(oauth_settings):
    ...
    with TestClient(app) as client:
        huge = client.post(
            "/register",
            content=b'{"redirect_uris":["https://client.example/callback"],"pad":"'
            + b"x" * (20 * 1024)
            + b'"}',
...
def test_registration_quota_evicts_unused_clients_and_rejects_when_full(
    ...
    monkeypatch.setattr("app.mcp_oauth.constants.OAUTH_CLIENT_MAX_COUNT", 2)
    app = create_app(oauth_settings)
    with TestClient(app) as client:
...
def test_authorization_transactions_are_purged_and_bounded(oauth_settings, monkeypatch):
    monkeypatch.setattr("app.mcp_oauth.constants.OAUTH_TRANSACTION_MAX_COUNT", 2)
    monkeypatch.setattr("app.mcp_oauth.constants.OAUTH_TRANSACTION_MAX_PER_CLIENT", 2)
    app = create_app(oauth_settings)
    resource = oauth_mcp_resource(oauth_settings.public_base_url or "")
    with TestClient(app) as client:
        registered = register_test_client(client)
        ...
        assert MCP_OAUTH_CONSENT_PATH in after_purge.headers["location"]
...
def test_authorization_transaction_quota_is_per_client(oauth_settings, monkeypatch):
    monkeypatch.setattr("app.mcp_oauth.constants.OAUTH_TRANSACTION_MAX_COUNT", 8)
    monkeypatch.setattr("app.mcp_oauth.constants.OAUTH_TRANSACTION_MAX_PER_CLIENT", 1)
    app = create_app(oauth_settings)
    resource = oauth_mcp_resource(oauth_settings.public_base_url or "")
    with TestClient(app) as client:
        first = register_test_client(client, client_name="one")
        ...
        assert MCP_OAUTH_CONSENT_PATH in other.headers["location"]
        assert _table_count(oauth_settings.database_path, "oauth_auth_transactions") == 2


def test_registration_retention_purges_unused_clients(oauth_settings, monkeypatch):
    monkeypatch.setattr("app.mcp_oauth.constants.OAUTH_CLIENT_UNUSED_RETENTION_SECONDS", 0)
    app = create_app(oauth_settings)
    with TestClient(app) as client:
...
async def test_oauth_audit_insert_prunes_old_rows_and_caps_newest(database, monkeypatch):
    monkeypatch.setattr("app.mcp_oauth.constants.OAUTH_AUDIT_MAX_COUNT", 3)
    monkeypatch.setattr("app.mcp_oauth.constants.OAUTH_AUDIT_RETENTION_SECONDS", 3600)
